...
if __name__ == "__main__":
    ...
    def get_polar_assisting_name(approx_method, osh, alpha, weight_min, margin, random=False):
        if approx_method == 'softmax':
            if weight_min == 0.5:
    ...
    def get_generalized_mil_names(angles, sigmas, margin, random=False):
        folders = [
            f'residual_parallel_approx_focal_{angles[0][0]}_{angles[0][1]}_expsumr={sigmas[0]}_unary_pair_margin={margin}',
            f'residual_parallel_approx_focal_{angles[1][0]}_{angles[1][1]}_explogs={sigmas[1]}_unary_pair_margin={margin}',
        ]
        if random:
            folders = [f"{f}_random" for f in folders]
        return folders

    def get_generalized_mil_name(angles, sigma, margin, mode, random=False):
        if mode == 'softmax':
            mode_name = 'expsumr'
        else:
            mode_name = 'explogs'
        folder = f'residual_parallel_approx_focal_{angles[0]}_{angles[1]}_{mode_name}={sigma}_unary_pair_margin={margin}'
        if random:
            folder = f"{folder}_random"
        return folder
    ...
    folders = [
        ...
        ### polar mil
        get_polar_name('softmax', [90, 30], 0.5, 0.6, margin),
        get_polar_name('quasimax', [60, 30], 0.5, 0.5, margin),

        ### proposed
        get_polar_assisting_name('softmax', [90, 20], 2, 0.5, margin),
        get_polar_assisting_name('quasimax', [120, 20], 2, 0.4, margin),
    ]
    # margin = 5
    # folders = [
    #     ### baseline
    #     f'residual_all_unary_pair_margin={margin}',

    #     ### generalized mil
    #     get_generalized_mil_name([40,10], 6, margin, 'softmax'),
    #     get_generalized_mil_name([40,20], 8, margin, 'quasimax'),

    #     ### polar mil
    #     get_polar_name('softmax', [90, 10], 0.5, 0.5, margin),
    #     get_polar_name('quasimax', [120, 20], 0.5, 0.8, margin),

    #     ### proposed
    #     get_polar_assisting_name('softmax', [120, 10], 1, 0.5, margin),
    #     get_polar_assisting_name('quasimax', [120, 10], 0.5, 0.5, margin),
    # ]    
    # margin = 10
    # folders = [
    #     ### baseline
    ...
    #     ### generalized mil
    #     get_generalized_mil_name([40,10], 8, margin, 'softmax'),
    #     get_generalized_mil_name([40,10], 4, margin, 'quasimax'),

    #     ### polar mil
    #     get_polar_name('softmax', [120, 30], 0.5, 0.5, margin),
    #     get_polar_name('quasimax', [60, 10], 1, 0.5, margin),

    #     ### proposed
    #     get_polar_assisting_name('softmax', [60, 30], 0.5, 0.5, margin),
    #     get_polar_assisting_name('quasimax', [90, 10], 0.5, 0.8, margin),
    # ]  
    # margin = 10
    # random = True
    # folders = [
    #     ### baseline
    #     f'residual_all_unary_pair_margin={margin}_random',

    #     ### generalized mil
    #     get_generalized_mil_name([60,30], 8, margin, 'softmax', random),
    #     get_generalized_mil_name([40,20], 4, margin, 'quasimax', random),

    #     ### polar mil
    #     get_polar_name('softmax', [120, 30], 0.5, 0.5, margin, random),
    #     get_polar_name('quasimax', [120, 40], 1, 0.2, margin, random),

    #     ### proposed
    #     get_polar_assisting_name('softmax', [120, 10], 1, 0.5, margin, random),
    #     get_polar_assisting_name('quasimax', [120, 10], 2, 0.5, margin, random),
    # ]
